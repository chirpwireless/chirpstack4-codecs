...
function decodeUplink(input) {
  if (input.bytes.length < 1 || (input.bytes[0] & 0x9F) != 0x9F) {
    return {data: {error: 'payload corruption'}};
  }
  var res = Decode(input.fPort, input.bytes);
  if (Math.abs(res.latitude) > 90 || Math.abs(res.longitude) > 180) {
    return {data: {error: 'coords corruption'}};
  }
  res.koterberg = Math.hypot((res.latitude - 51.85533), (res.longitude - 9.32534)*Math.cos(res.latitude * Math.PI / 180))*111.3;
  return {data: res};
}
